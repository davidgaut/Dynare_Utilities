...
for jj=1:nvar_exo        
    y2(:,:,jj) = irf(Model, options_,oo_.dr,cs(Model.exo_names_orig_ord,i_var_exo(jj)), horizon_est, options_.drop, ...
...
DSGE_IRF = DSGE_IRF(1:horizon_est,:,:) * scale_factor;
psitheta = DSGE_IRF(:);
...
% ------------------------------------------------------------------------------
fval = - (likelihood + include_prior*lnprior);
