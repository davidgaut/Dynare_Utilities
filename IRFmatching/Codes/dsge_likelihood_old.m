...
elseif isfield(DynareOptions,'frequency_estimation')
    [fval,lik] = logFreq(Model.Iy,DynareOptions.estim_frequency,options_.varobs,DynareResults.dr,Model,lnprior);
    DLIK       = [-lnprior; lik(:)];
...
[CVD, ~] = conditional_variance_decomposition(StateSpaceModel, [1 16 32], [93]); % for c
if ~all(mean(CVD(:,:,1),2) > [0.0 0.35 0.0]')
        fval = Inf;
        info(1) = 86;
        info(2) = 1.0222;
        info(4) = info(2);
