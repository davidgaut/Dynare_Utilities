...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isfield(DynareOptions,'irfs_match_estimation') && isfield(DynareOptions,'BLD_Estimation')
% persistent xtc
% 
% if isempty(xtc)
%     xtc = xparam1;
% end
% 
% name_ss_mod    = {'chi_p' 'kappa_p' 'xi_p' 'tau_p' 'sigma1_p' 'sigma2_p' 'sigma3_p' 'mub_p' 'mul_p' 'b_p'}';
% 
% xidiff = xparam1 - xtc;
% 
...
elseif isfield(DynareOptions,'frequency_estimation')
    if DynareResults.dr.frequency_estimation_init
       [Model.Iy,DynareOptions.varobs_idx] = frequency_estimation_init(DatasetInfo,Model,DynareResults,DynareOptions);
        DynareResults.dr.frequency_estimation_init = 0;
    end
    [fval,lik] = logFreq(Model.Iy,length(DatasetInfo.rawdata),DynareOptions.estim_frequency,DynareOptions.varobs_idx,DynareResults.dr,Model,lnprior);
    DLIK       = [-lnprior; lik(:)];
...
%-----------------------------------------------------
if 0
StateSpaceModel.number_of_state_equations = Model.endo_nbr;
StateSpaceModel.number_of_state_innovations = Model.exo_nbr;
StateSpaceModel.sigma_e_is_diagonal = Model.sigma_e_is_diagonal;

[StateSpaceModel.transition_matrix, StateSpaceModel.impulse_matrix] = ...
            kalman_transition_matrix(DynareResults.dr,(1:Model.endo_nbr)',Model.nstatic+(1:Model.nspred)',Model.exo_nbr);
        
StateSpaceModel.state_innovations_covariance_matrix = Model.Sigma_e;
StateSpaceModel.order_var = DynareResults.dr.order_var;
StateSpaceModel.measurement_error = Model.H;
StateSpaceModel.observable_pos = DynareOptions.varobs_id;
...
elseif 0
% IRF Signs Restrictions
%-------------------------
    if DynareResults.dr.ghu(DynareResults.dr.inv_order_var(strcmp(Model.endo_names,'qh_')),strcmp(Model.exo_names,'e_nu')) > 0
        fval = Inf;
        info(1) = 86;
        info(2) = 1.0222;
        info(4) = info(2);
