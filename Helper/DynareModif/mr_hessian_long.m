function [hessian_mat, gg, htol1, ihh, hh_mat0, hh1, hess_info] = mr_hessian_long(x,func,penalty,hflag,htol0,hess_info,varargin)
...
%     end
    ...
    hessian_mat = hessian('dsge_likelihood',x,varargin{3}.gstep, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5}, varargin{6}, varargin{7}, varargin{8});
%     hessian(objective_function, xparam1, options_.gstep, dataset_, dataset_info, options_, M_, estim_params_, bayestopt_, bounds,oo_);
%     hessian_mat = reshape(hessian_mat, sqrt(length(hessian_mat)), sqrt(length(hessian_mat)));
