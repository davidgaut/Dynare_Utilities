function myoutput = posterior_sampler_core_para(myinputs,fblck,nblck,whoiam, ThisMatlab)
...
OutputPara = repmat(struct('OutputFileName',[],'LastLogPost',[],'AcceptanceRatio',[],'FunctionEvalPerIteration',[],'LastSeeds',[],'NewFile',[]),nblck-fblck+1,1);

parfor curr_block = fblck:nblck
    options_para = options_;
    LastSeeds=[];
    LastParameters=[];
    LastLogPost=[];
    ...
    try
        ...
        if ~isoctave
            options_para = set_dynare_seed_para(options_para.DynareRandomStreams.algo, options_para.DynareRandomStreams.seed,options_para);
        else
            options_para =set_dynare_seed_para(options_para.DynareRandomStreams.seed+curr_block,[],options_para);
        end     
        ...
        set_dynare_seed_para(options_para.DynareRandomStreams.seed+curr_block,[],options_para);
    end
    if (options_para.load_mh_file~=0) && (fline(curr_block)>1) && OpenOldFile(curr_block) %load previous draws and likelihood
        LoadMat = load([BaseName '_mh' int2str(NewFile(curr_block)) '_blck' int2str(curr_block) '.mat'])
        x2=LoadMat.x2; LastSeeds=LoadMat.LastSeeds; logpo2=LoadMat.logpo2;
...
%         hh = dyn_waitbar_para({prc0,whoiam,Parallel,running_text,newString,options_.parallel(ThisMatlab)},whoiam,options_,[bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ')...']);
        hh = waitbar(0,'0');  %CHANGE
    else
        refresh_rate = sampler_options.serial_bar_refresh_rate;
        bar_title = sampler_options.serial_bar_title;
        hh = dyn_waitbar_para(0,whoiam,options_,[bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ')...']);
%         set(hh,'Name',bar_title);
    ...
    feval_this_chain = 0;
    feval_this_file = 0;
    draw_index_current_filePara = fline(curr_block); %get location of first draw in current block
    draw_iter = 1;

    while draw_iter <= nruns(curr_block)
        
        if rem(100,draw_iter) == 0
        perc    = (draw_iter/nruns(curr_block))*100;
        Pstatus = ['Chain n°',num2str(curr_block), ':   ', num2str(perc)];
%        	fprintf(repmat('\b', 1, length(Pstatus)))
        disp(Pstatus)       
        end
        ...
        x2(draw_index_current_filePara,:) = par;
        last_draw(curr_block,:) = par;
        logpo2(draw_index_current_filePara) = logpost;
        ...
        if mod(draw_iter, refresh_rate)==0
            whoiamCHANGE = 0; %CHANGE
            if accepted_draws_this_chain/draw_iter==1 && sum(neval)>1
                dyn_waitbar_para(prtfrc,whoiamCHANGE,options_,hh,[bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ') ' sprintf('Function eval per draw %4.3f', feval_this_chain/draw_iter)]);
            else
                dyn_waitbar_para(prtfrc,whoiamCHANGE,options_,hh,[bar_title ' (' int2str(curr_block) '/' int2str(options_.mh_nblck) ') ' sprintf('Current acceptance ratio %4.3f', accepted_draws_this_chain/draw_iter)]);
            end
            if save_tmp_file
                ...
                pdyn_save([BaseName '_mh_tmp_blck' int2str(curr_block) '.mat'],x2,logpo2,LastSeeds);
            end
        end
        if (draw_index_current_filePara == InitSizeArray(curr_block)) || (draw_iter == nruns(curr_block)) % Now I save the simulations, either because the current file is full or the chain is done
            [LastSeeds.(['file' int2str(NewFile(curr_block))]).Unifor, LastSeeds.(['file' int2str(NewFile(curr_block))]).Normal] = get_dynare_random_generator_state();
            if save_tmp_file
                delete([BaseName '_mh_tmp_blck' int2str(curr_block) '.mat']);
            end
            pdyn_save([BaseName '_mh' int2str(NewFile(curr_block)) '_blck' int2str(curr_block) '.mat'],x2,logpo2,LastSeeds);
...
%                 record.LastLogPost(curr_block) = logpo2(end);
                LastParameters = x2(end,:);
                LastLogPost = logpo2(end);
            end
...
%             % initialization of next file if necessary
            if InitSizeArray(curr_block)
                ...
                draw_index_current_filePara = 0;
            end
        end
        draw_iter=draw_iter+1;
        draw_index_current_filePara = draw_index_current_filePara + 1;
...
%     record.FunctionEvalPerIteration(curr_block) = feval_this_chain/(draw_iter-1);
	  AcceptanceRatio          = accepted_draws_this_chain/(draw_iter-1);
      FunctionEvalPerIteration = feval_this_chain/(draw_iter-1);
...
%     OutputFileName(block_iter,:) = {[MetropolisFolder,filesep], [ModelName '_mh*_blck' int2str(curr_block) '.mat']};
    OutputFileNamePara = {[MetropolisFolder,filesep], [ModelName '_mh*_blck' int2str(curr_block) '.mat']};
    
    % Output
    OutputPara(curr_block).LastParameters            = LastParameters;
    OutputPara(curr_block).LastLogPost               = LastLogPost;
    OutputPara(curr_block).AcceptanceRatio           = AcceptanceRatio;
    OutputPara(curr_block).FunctionEvalPerIteration  = FunctionEvalPerIteration;
    OutputPara(curr_block).LastSeeds.Unifor          = LastSeeds.Unifor;
    OutputPara(curr_block).LastSeeds.Normal          = LastSeeds.Normal;
    OutputPara(curr_block).OutputFileName            = OutputFileNamePara;
    OutputPara(curr_block).NewFile                   = NewFile(curr_block);
	OutputPara(curr_block).draw_index_current_filePara   = draw_index_current_filePara;
...
for  curr_block = fblck:nblck
    ...
    block_iter=block_iter+1;
    record.FunctionEvalPerIteration(curr_block,:)     = OutputPara(curr_block).FunctionEvalPerIteration;
    record.AcceptanceRatio(curr_block)                = OutputPara(curr_block).AcceptanceRatio;
    
    record.LastParameters(curr_block,:)     = OutputPara(curr_block).LastParameters;
    record.LastLogPost(curr_block)          = OutputPara(curr_block).LastLogPost;
    record.LastSeeds(curr_block).Unifor     = OutputPara(curr_block).LastSeeds.Unifor;
    record.LastSeeds(curr_block).Normal     = OutputPara(curr_block).LastSeeds.Normal;
    draw_index_current_file                 = OutputPara(curr_block).draw_index_current_filePara;
    NewFile                                 = OutputPara(curr_block).NewFile;
    OutputFileName(block_iter,:)            = OutputPara(curr_block).OutputFileName;
end
...
function pdyn_save(fname,x2,logpo2,LastSeeds)
save(fname,'x2','logpo2','LastSeeds')
