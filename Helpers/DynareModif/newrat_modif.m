...
flagit=2;  % mode of computation of hessian in each iteration
ftol=ftol0;
gtol=1.e-3;
htol=htol_base;
htol0=htol_base;
...
% initialize mr_gstep and mr_hessian_long

outer_product_gradient=1;
if isempty(hh)
    [dum, gg, htol0, igg, hhg, h1, hess_info]=mr_hessian_long(x,func0,penalty,flagit,htol,hess_info,varargin{:});
    if isempty(dum)
...
H = igg;
disp_verbose(varargin{6}.name(find(abs(gg)>1e2)),Verbose) 
...
while norm(gg)>gtol && check==0 && jit<nit
    ...
    [fval,x0,fc,retcode] = csminit1(func0,xparam1,penalty,fval0(icount),gg,0,H,Verbose1,varargin{:});
    if igrad
        [fval1,x01,fc,retcode1] = csminit1(func0,x0,penalty,fval,gg,0,inx,Verbose1,varargin{:});
        if (fval-fval1)>1
    ...
    if length(find(ig))<nx
        ...
        [fvala,x0,fc,retcode] = csminit1(func0,x0,penalty,fval,ggx,0,iggx,Verbose1,varargin{:});
    end
    x0 = check_bounds(x0,bounds);
    [fvala, x0, ig] = mr_gstep(h1,x0,bounds,func0,penalty,htol0,Verbose1,Save_files,varargin{:});
    x0 = check_bounds(x0,bounds);
    nig=[nig ig];
    ...
    if (fval0(icount)-fval)<ftol && flagit==0
        ...
        ihh=diag(1./(diag(hhg)));
        [fval2,x0,fc,retcode2] = csminit1(func0,x0,penalty,fval,gg,0,ihh,Verbose1,varargin{:});
        x0 = check_bounds(x0,bounds);
        if (fval-fval2)>=ftol
    ...
    if (fval0(icount)-fval)<ftol && flagit==0
        ...
        ihh0=inx.*1.e-4;
        [fval3,x0,fc,retcode3] = csminit1(func0,x0,penalty,fval,gg,0,ihh0,Verbose1,varargin{:});
        x0 = check_bounds(x0,bounds);
        if (fval-fval3)>=ftol
    ...
    if (fval0(icount)-fval)<ftol
        ...
        if analytic_derivation
            ...
            if flagit==2
                hh=hh0;
            elseif flagg>0
                [dum, gg, htol0, igg, hhg, h1, hess_info]=mr_hessian_long(xparam1,func0,penalty,flagg,ftol0,hess_info,varargin{:});
...
%                 end
        ...
        disp_verbose(varargin{6}.name(find(abs(gg)>100)),Verbose) 
        ...
        if norm(x(:,icount)-xparam1)>1.e-12 && analytic_derivation==0
            ...
            [dum, gg, htol0, igg, hhg, h1, hess_info]=mr_hessian_long(xparam1,func0,penalty,flagit,htol,hess_info,varargin{:});
            keepH = reshape(dum,nx,nx);
            if isempty(dum)
                outer_product_gradient=0;
            end
